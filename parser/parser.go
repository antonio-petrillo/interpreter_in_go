...
func (p *Parser) parseCallArguments() (args []ast.Expression) {
...
func (p *Parser) parseFunctionParameters() []*ast.Identifier {
	...
	if !p.expectPeek(token.RPAREN) {
		msg := "expected closing parentheses after function identifiers declaration"
...
func (p *Parser) peekError(t token.TokenType) {
	msg := fmt.Sprintf("expected next token to be %q, got %q", t, p.peekToken.Type)
	p.errors = append(p.errors, msg)
}
...
func (p *Parser) parseGroupedExpression() ast.Expression {
	...
	if !p.expectPeek(token.RPAREN) {
		msg := fmt.Sprintf("expected closing parens ')' but got %T", p.curToken.Type)
		p.errors = append(p.errors, msg)
		return nil
	}
...
func (p *Parser) parseIfExpression() ast.Expression {
	...
	if !p.expectPeek(token.LPAREN) {
		msg := "expected opening parens in if condition"
		p.errors = append(p.errors, msg)
		return nil
	}
	...
	if !p.expectPeek(token.RPAREN) {
		msg := "expected closing parens in if condition"
		p.errors = append(p.errors, msg)
		return nil
	}

	if !p.expectPeek(token.LBRACE) {
		msg := "expected opening squirly parens after if condition"
		p.errors = append(p.errors, msg)
		return nil
	}
	...
	if p.peekTokenIs(token.ELSE) {
		...
		if !p.expectPeek(token.LBRACE) {
			msg := "expected opening squirly parens after else"
			p.errors = append(p.errors, msg)
			return nil
		}
